...
//1. 首先进行填充， 补充二进制 10000... 补足  %512 余数 为448, 之后添加补充前长度 (bit长度) 此处应该使用小端字节序， 比如长度 0x28,
//	应该是 0x280000..., 无论机器本身的endianess
//	注意：填充是必须的，就算当前 %512 余数为 448 也需要填充 512bit
//2. 以512bit 为一个大组，对于每个大组进行计算，不断更新初始的 a/b/c/d, 在一个大组中，划分16个小组，来进行计算。这里划分16个小组的时候，事实上
//	将之前以字符串方式存入的数据，以数值的方式读出了。
//	注意： FF GG HH II 函数中 << 表示的是循环左移， 非左移运算
//3. 将step 2中 a/b/c/d 组合成为 128bit 结果即可, 这里需要按照小端序来链接 a/b/c/d

//注意： 这里是使用little endianess 来构建的（比如长度），而不用考虑机器本身的endianess
//具体参考 byteorder.go 注释的最后一部分：
...
//因此这里md5算法无论用于大端pc还是小端pc 都是正确的，就是因为md5知道无论哪种机器上都是使用小端序表示对应数据部分的。
